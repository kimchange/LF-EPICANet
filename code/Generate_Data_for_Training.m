...
% src_data_path = 'I:/LF-DFnet-master/Datasets/';
src_data_path = '/mnt3/LuoXin/datasets/';
src_data_path = ['/root/user/jmc/kimchange/LFSR/LFSSR_data/datasets/'];
    ...
    for index_scene = 1 : num_scene 
        ...
        for h = 1 : stride : H - patchsize + 1
            for w = 1 : stride : W - patchsize + 1
                ...
                SavePath = ['/root/user/jmc/kimchange/LFSR/LFSSR_data/TrainingData_', num2str(stride), '_',num2str(factor),'_',num2str(factor),'xSR_', num2str(angRes), 'x' , num2str(angRes),'/' ];
                ...
                h5create(SavePath_H5, '/data', size(Lr_SAI_y), 'Datatype', 'single');
                h5create(SavePath_H5, '/label', size(Hr_SAI_y), 'Datatype', 'single');
                try
                    h5write(SavePath_H5, '/data', single(Lr_SAI_y), [1,1], size(Lr_SAI_y));
                    h5write(SavePath_H5, '/label', single(Hr_SAI_y), [1,1], size(Hr_SAI_y));
                catch
                    pause(5);
                    h5write(SavePath_H5, '/data', single(Lr_SAI_y), [1,1], size(Lr_SAI_y));
                    h5write(SavePath_H5, '/label', single(Hr_SAI_y), [1,1], size(Hr_SAI_y));
